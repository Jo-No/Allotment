...
class AddTaskActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        editTask?.let { task ->
            ...
            edit_month.equals(task.month.toString())
            task_completed.equals(task.completed)
        }
        ...
        submit.setOnClickListener {
            when {
                edit_task_description.text?.toString().isNullOrBlank() -> edit_task_description.error = "Please enter a description"
                    ...
                    val task = Task(description = edit_task_description.text.toString(), month = edit_month.toString(), id = editTask?.id)
